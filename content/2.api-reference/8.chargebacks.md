...
| `merchantId` | `string` | ID of the merchant (starts with `merchant_`). |
...
| `orderId` | `string \| null` | ID of the credit note order created for this chargeback (starts with `order_`). Only present after the chargeback is processed. |
