...
Every billable model can be linked to a Vatly customer. Customers are created automatically by Vatly during checkout, then synced to your application via webhooks. You can also create customers explicitly if needed.
...
When a user goes through checkout without an existing Vatly customer ID, Vatly creates the customer automatically. The customer ID is synced back to your application via webhooks.
